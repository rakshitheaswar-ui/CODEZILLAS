...
def load_data():
    ...
    try:
        ...
        df = pd.read_parquet('/Users/dhineashkumar/Desktop/inventory-gpt/data/m5_feature_engineered_2014.parquet')
        ...
        actuals_df = pd.read_parquet('/Users/dhineashkumar/Desktop/inventory-gpt/data/m5_feature_engineered_2015.parquet')  
...
def chatbot_page():
    ...
    # UI toggle: show code context under answers
    with st.sidebar:
        explain_with_context = st.toggle("Explain with code context", value=st.session_state.get("explain_with_context", False))
        st.session_state.explain_with_context = explain_with_context
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                ...
                if isinstance(response, dict):
                    ...
                    if st.session_state.get("explain_with_context"):
                        sources = response.get("sources") or []
                        if sources:
                            with st.expander("Show code context", expanded=False):
                                for s in sources:
                                    st.markdown(f"- `{s['path']}` (score: {s['score']:.3f})")
